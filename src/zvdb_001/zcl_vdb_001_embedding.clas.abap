CLASS zcl_vdb_001_embedding DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS new
      ...
      RETURNING
        VALUE(ro_) TYPE REF TO zcl_vdb_001_embedding .
    METHODS answer
      IMPORTING
        !iv_       TYPE string
      EXPORTING
  ...
  PRIVATE SECTION.
    ...
    "   DATA: mo_sdk_chat  TYPE REF TO zif_peng_azoai_sdk.
    DATA: mo_lib TYPE REF TO zcl_vdb_001_lib.
    DATA: ms_aoi TYPE zcl_oai_01_dotenv=>ts_openai.
    METHODS: constructor
      IMPORTING iv_ TYPE zvdb_001_vector-bid.
ENDCLASS.



CLASS ZCL_VDB_001_EMBEDDING IMPLEMENTATION.
...
*     Display the results.
  ...
  METHOD constructor.
    ...
    ms_aoi = VALUE zcl_oai_01_dotenv=>ts_openai(
      api_url       = mo_env->v( k = 'API_URL' )
      api_ver       = mo_env->v( k = 'API_VER' )
      api_key       = mo_env->v( k = 'API_KEY' )
      depid         = mo_env->v( k = 'DEPID'   )
    ).

    mo_sdk_lite = zcl_oai_01_sdk_lite=>new( ms_aoi ).
  ...
  METHOD new.
    ro_ = NEW zcl_vdb_001_embedding( iv_ ).
