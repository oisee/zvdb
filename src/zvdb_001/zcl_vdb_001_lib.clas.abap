CLASS zcl_vdb_001_lib DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE .
  ...
  PUBLIC SECTION.
    ...
    TYPES ts_embedding TYPE zif_peng_azoai_sdk_typinternal=>ty_coordinate .
    TYPES tt_embedding TYPE zif_peng_azoai_sdk_typinternal=>ty_embedding-embedding .
    TYPES ts_vector TYPE zvdb_001_vector .
    TYPES:
    ...
    TYPES:
      BEGIN OF ts_hash,
        hh1  TYPE zvdb_001_vector-hh1,
        hh2  TYPE zvdb_001_vector-hh2,
        hr1  TYPE zvdb_001_vector-hr1,
        hr2  TYPE zvdb_001_vector-hr2,
*       hff2 TYPE ty_b16,
*       hff3 TYPE ty_b24,
*       hff6 TYPE ty_b48,
      END OF ts_hash .
...
*    TYPES:
*      BEGIN OF ts_query,
*        id   TYPE zvdb_001_vector-id,
*        rank TYPE i,
*        xor  TYPE i,
*        "       jc          TYPE f,
*        "       or_and_diff TYPE i, " or - rank = xor -> just use xor
*        p    TYPE zvdb_001_vector-p,
*      END OF ts_query .
*    TYPES:
...
*        id   TYPE zvdb_001_vector-id,
*        rank TYPE i,
*        q1b  TYPE zvdb_001_vector-q1b,
*        ip   TYPE zvdb_001_vector-ip,
*      END OF ts_q .
*    TYPES:
*      tt_q TYPE STANDARD TABLE OF ts_q WITH KEY id .
    ...
    DATA mv_bid TYPE zvdb_001_vector-bid .
...
*    DATA mt_hash_log TYPE tt_vector .
    ...
    CLASS-METHODS new
      IMPORTING
        !iv_       TYPE zvdb_001_vector-bid
        !is_       TYPE ts_index_bypass OPTIONAL
          PREFERRED PARAMETER !iv_
      RETURNING
        VALUE(ro_) TYPE REF TO zcl_vdb_001_lib .
    METHODS bf
      IMPORTING
        !ix_       TYPE tv
      RETURNING
    ...
    METHODS dp_1536
      IMPORTING
        !ix_0      TYPE zvdb_001e_raw192
        !ix_1      TYPE zvdb_001e_raw192
      RETURNING
        VALUE(rv_) TYPE i .
    METHODS dp_1536_bak
      IMPORTING
    ...
    METHODS read_vector
      IMPORTING
        !iv_       TYPE zvdb_001_vector-id
      RETURNING
        VALUE(rs_) TYPE zvdb_001_vector .
    ...
    METHODS recalibrate_hash_and_reindex
      IMPORTING
        !iv_         TYPE string DEFAULT 'MEDIUM'
        !iv_treshold TYPE f DEFAULT '0.6'
        !iv_bid      TYPE zvdb_001_vector-bid
        !is_         TYPE ts_index_bypass OPTIONAL
      RETURNING
        VALUE(rt_)   TYPE tt_set .
    METHODS regen_hash .
...
CLASS zcl_vdb_001_lib IMPLEMENTATION.
...
*   gt_hamming_24 = init_hamming_24bit( ).
  ...
  METHOD init_save_hp.
    ...
    DATA: ltr_id TYPE RANGE OF zvdb_001_vector-id.
    ...
    MODIFY zvdb_001_vhs FROM TABLE mt_hh.
    MODIFY zvdb_001_vhs FROM TABLE mt_hr.
...
*--------------------------------------------------------------------*
    ...
    MODIFY zvdb_001_set FROM TABLE rt_.
    COMMIT WORK AND WAIT.
  ENDMETHOD.
...
*     lr_v->hff3 = ls_h-hff3.
  ...
  METHOD reset_hp.
    ...
    DELETE FROM zvdb_001_vhs WHERE bid = @mv_bid AND id IS NOT NULL.
  ...
  METHOD save_vector.
    ...
    ENDIF.
    MODIFY zvdb_001_vector FROM ls_.
