...
*&
*&---------------------------------------------------------------------*
REPORT zvdb_001_query_01 MESSAGE-ID zcol_177.
...
PARAMETERS: q TYPE guid DEFAULT '000D3A7A9A961EDE9CD283424391F4F2' MATCHCODE OBJECT zvdb_001_vector_sh. "ABAP is programming language for SAP R/3.

*PARAMETERS: p_d AS CHECKBOX.
PARAMETERS: p_bid TYPE zvdb_001_vector-bid MATCHCODE OBJECT zvdb_001_vector_sh.
...
*PARAMETERS: p_h1_1 RADIOBUTTON GROUP r1 DEFAULT 'X' USER-COMMAND r01 MODIF ID r01.
*PARAMETERS: p_h1_2 RADIOBUTTON GROUP r1 MODIF ID r01.
*PARAMETERS: p_h1_3 RADIOBUTTON GROUP r1 MODIF ID r01.
...
*PARAMETERS: p_h1_3 AS CHECKBOX MODIF ID r01.
...
*PARAMETERS: p_h2_1 RADIOBUTTON GROUP r2 DEFAULT 'X' USER-COMMAND r02 MODIF ID r02.
*PARAMETERS: p_h2_2 RADIOBUTTON GROUP r2 MODIF ID r02.
...
*PARAMETERS: p_h3_1 RADIOBUTTON GROUP r3 DEFAULT 'X' USER-COMMAND r03 MODIF ID r03.
*PARAMETERS: p_h3_2 RADIOBUTTON GROUP r3 MODIF ID r03.
...
*PARAMETERS: p_h4_1 RADIOBUTTON GROUP r4 DEFAULT 'X' USER-COMMAND r04 MODIF ID r04.
*PARAMETERS: p_h4_2 RADIOBUTTON GROUP r4 MODIF ID r04.
...
CLASS lcl_ IMPLEMENTATION.
  METHOD query.
    DATA(lo_sw) = zcl_vdb_stopwatch=>new( ).

    DATA(lo_l) = zcl_vdb_001_lib=>new( iv_ = p_bid ).
...
*    cl_demo_output=>write( |In total:{ ls_batch-lines * lines( lt_q ) } gives average time ms per line: { ls_batch-average / lines( lt_q ) }| ).
    ...
    DATA(lo_alv) = NEW zcl_eui_alv(
         ...
         it_mod_catalog = VALUE #(
           ( fieldname = 'Q1B'    tech    = 'X' )

           ( fieldname = 'HH1'     coltext = 'HH1'     )
           ( fieldname = 'HH2'     coltext = 'HH2'     )
           ( fieldname = 'HR1'     coltext = 'HR1'     )
           ( fieldname = 'HR2'     coltext = 'HR2'     )
         )
*      is_layout      = is_layout
...
*    SELECT id, q1b, p
*      FROM zvdb_001_v
*      INTO TABLE @DATA(lt_v0)
*      WHERE id IS NOT NULL.
*
*    DATA: lt_vec TYPE STANDARD TABLE OF zvdb_001_vector WITH KEY id.
*    MOVE-CORRESPONDING lt_v0 TO lt_vec.
*
*    MODIFY zvdb_001_vector FROM TABLE lt_vec.
*
*    COMMIT WORK AND WAIT.
*    RETURN.
    DATA(lo_l) = zcl_vdb_001_lib=>new( p_bid ).
*    data(lt_set0) = lo_l->recalibrate_hash_and_reindex(
*      EXPORTING
*        iv_         = 'MEDIUM' " Character Field Length 1
*        iv_treshold = conv f( p_seed / '10.0' )
*        iv_seed     = p_seed  " 2 byte integer (signed)
*    ).
*    lo_l->reindex_all( p_seed ).
...
* #TODO:search by flags (checkboxes, or, highlite indicate what was retrieval for each)
    DATA:lt_bf    TYPE zcl_vdb_001_lib=>tt_vector.
    DATA:lt_h8    TYPE zcl_vdb_001_lib=>tt_vector.
    DATA:lt_h8ha  TYPE zcl_vdb_001_lib=>tt_vector.
    DATA:lt_h16   TYPE zcl_vdb_001_lib=>tt_vector.
    DATA:lt_h16ha TYPE zcl_vdb_001_lib=>tt_vector.
    DATA:lt_r8    TYPE zcl_vdb_001_lib=>tt_vector.
    DATA:lt_r8ha  TYPE zcl_vdb_001_lib=>tt_vector.
    DATA:lt_r16   TYPE zcl_vdb_001_lib=>tt_vector.
    DATA:lt_r16ha TYPE zcl_vdb_001_lib=>tt_vector.
...
*      lr_alv->fh     = lo_l->hash_fast_fold( lr_alv->q1b )-hff2.
*      lr_alv->fh_rank = lo_l->dp(
*                          ix_0 = lv_q_fh-hff2
*                          ix_1 = lr_alv->fh
*                        ).
*
*      lr_alv->rank2 = lr_alv->dh1 + lr_alv->dh2 + lr_alv->dr1 + lr_alv->dr2.

    ENDLOOP.
*   DATA(lv_arank_scaled) = lo_l->mv_rank_h * '1.05'.
*   MODIFY lt_v FROM VALUE #( ip = 55 ) TRANSPORTING ip WHERE rank < ( lo_l->mv_rank_h ).
*   MODIFY lt_v FROM VALUE #( ip = 55 ) TRANSPORTING ip where xor > ( lo_l->mv_axor_h ).
*   MODIFY lt_v FROM VALUE #( ip = 55 ) TRANSPORTING ip WHERE rank < ( lv_arank_scaled ).
*   MODIFY lt_v FROM VALUE #( ip = 55 ) TRANSPORTING ip WHERE xor > ( lv_axor_scaled ).
    ...
    DATA(lo_alv) = NEW zcl_eui_alv(
      ...
      it_mod_catalog = VALUE #(
        ...
        ( fieldname = 'DH1'     coltext = 'DH1'     )
        ( fieldname = 'DH2'     coltext = 'DH2'     )
        ( fieldname = 'DR1'     coltext = 'DR1'     )
        ( fieldname = 'DR2'     coltext = 'DR2'     )
        ( fieldname = 'RANK2'   coltext = 'RANK2 QF'  )

        ( fieldname = 'FH'          coltext = 'FH' )
        ( fieldname = 'FH_RANK'     coltext = 'FH Rank' )
        ( fieldname = 'FH_OR'       coltext = 'FH OR' )
        ( fieldname = 'FH_RANK_OR'  coltext = 'FHR OR' )
        ( fieldname = 'FH_AND'      coltext = 'FH AND' )
        ( fieldname = 'FH_RANK_AND' coltext = 'FHR AND' )

        ( fieldname = 'HH8'     coltext = 'HH8'     )
        ( fieldname = 'HH8HA'   coltext = 'HH8HA'   )
        ( fieldname = 'HH16'    coltext = 'HH16'    )
        ( fieldname = 'HH16HA'  coltext = 'HH16HA'  )
        ( fieldname = 'HR8'     coltext = 'HR8'     )
        ( fieldname = 'HR8HA'   coltext = 'HR8HA'   )
        ( fieldname = 'HR16'    coltext = 'HR16'    )
        ( fieldname = 'HR16HA'  coltext = 'HR16HA'  )
        ( fieldname = 'BF'      coltext = 'BF'      )
        ( fieldname = 'BF_ONLY' coltext = 'BF Only' )
      )
*      is_layout      = is_layout
*      is_variant     = is_variant
*      it_filter      = it_filter
...
*    cl_demo_output=>write( |lines: { lines( lt_v ) }| ).
*    cl_demo_output=>write( lt_v ).
*    cl_demo_output=>display( ).
...
*--------------------------------------------------------------------*
    ...
    DATA: lt_used TYPE zcl_vdb_001_lib=>tt_vector.
